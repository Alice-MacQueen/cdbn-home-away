...
```
Clearly, the Andean domestication group (Nueva Granada) are genetically distinct different. Jalisco can roll into Durango. 

These must be constrained axes.
...
```
Clearly, common sites have a lot of well-adapted varieties. This could be a side effect of how home was identified (blup/shrinkage), which penalizes rare sites. 
...
## Plot Variances 
```{r}
pltdf = names(anova_out) %>% 
  lapply(function(x) {
    best_model = which.min(aic_out[[x]][,1])
    best_model='home'
    subav = anova_out[[x]][[best_model]]
    subav$race = x
    subav$model = names(anova_out[[x]])[best_model]
...
variance_plot = 
  ...
  geom_col() +
  labs(title='a) Sources of Variance in Yield',
       y = 'Variance [%]',
       x = 'Race',
       fill = NULL) +
  theme_minimal()
...
## Yield ~ Year
```{r}
se_high = function(x) mean(x) + sd(x)/sqrt(length(x))
se_low = function(x) mean(x) - sd(x)/sqrt(length(x))
cat_to_num = function(x) as.numeric(as.character(x))

mm = lm(SY ~ cat_to_num(Year)*Race, data=df)

yield_yr = ggplot(df,
                  aes(x=cat_to_num(Year),
                      y=SY,
                      color=Race)) +
  ...
  geom_smooth(method='lm',
              formula=y~x) +
  labs(title = 'b) Yield Across Study Period',
       x = 'Year',
       y = 'Yield [kg/ha]') +
  theme_minimal()
